...
**Texture ranking & flow sequencing** (research-driven, 2026-07): candidates rank by feature-vector distance to the *exemplar centroid* (energy/danceability/valence/acousticness/instrumentalness averaged over the resolved exemplars) — the "popularity bypass" recommended by the MIR literature, done with provider features instead of audio embeddings. A **texture gate** additionally drops candidates acoustically alien to the centroid (e.g. a rap track mis-surfaced by a `genre:"hard bop"` search), never removing exemplars or so many that the pool can't fill 20. Vibe playlists enforce **one track per primary artist**: the neighborhood is widened to ~16 artists and the candidate pool is capped at one track per artist (exemplars exempt) so ~20 distinct artists exist to fill the list without backfilling duplicates. When no tempo target dictates cadence-first order, the final list is sequenced as a greedy minimum-transition-cost path over energy delta, octave-folded tempo delta, and Camelot key adjacency. Sequencing is presentation-only: living-playlist refresh and Replace-weakest consume the list prefix as best-N candidates and pass `sequence: false`.
