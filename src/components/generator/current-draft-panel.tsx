...
export function CurrentDraftPanel() {
  ...
  return (
    <aside className="w-64 sticky top-28 space-y-4">
      ...
      <div className="space-y-2 max-h-[400px] overflow-y-auto custom-scrollbar">
        {currentDraft.tracks.map((track) => (
          <div
            key={track.id}
            className="flex items-center gap-3 p-2 rounded-lg bg-surface-container/40 group"
          >
      ...
      <div className="pt-4 border-t border-white/5 space-y-3">
        <div className="flex justify-between text-xs text-on-surface-variant">
          ...
          <span className="font-bold text-primary">+{totalDuration()}</span>
        </div>

        <button
          type="button"
          onClick={() => router.push(`/builder/${currentDraft.id}`)}
          className="w-full py-3 bg-primary text-on-primary rounded-full font-bold text-sm flex items-center justify-center gap-2 active:scale-95 transition-transform"
        >
